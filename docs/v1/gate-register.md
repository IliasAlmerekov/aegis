...
| G7 | Snapshot and rollback flows are fail-closed and regression-tested across providers | in_progress | Ilias | G2 | `src/snapshot/git.rs` (rollback malformed/unknown hash path), `tests/full_pipeline.rs` (known-provider malformed id, unknown-provider path); `rtk cargo test --lib rollback_rejects_malformed_snapshot_id`, `rtk cargo test --lib rollback_errors_when_stash_entry_not_found`, `rtk cargo test --test full_pipeline rollback_with_` | 2026-04-15 | 30 | true | Ilias | Pack 02: G7 частично закрыт (git + CLI path), требуется завершить matrix покрытия остальных провайдеров. |
