...
| G3 | Safe-path performance expectations are documented and checked against a baseline | in_progress | Ilias | none | `docs/performance-baseline.md`, `perf/scanner_bench_baseline.toml`, `rtk cargo bench --bench scanner_bench`, `rtk cargo run --quiet --bin aegis_benchcheck -- --baseline perf/scanner_bench_baseline.toml --criterion-root target/criterion` | 2026-04-15 | 14 | true | Ilias | Pack 03 стартован; последняя проверка проходит без fail (all PASS по tracked benchmark-ам). |
