...
| G7 | Snapshot and rollback flows are fail-closed and regression-tested across providers | in_progress | Ilias | G2 | `src/snapshot/git.rs` (malformed/stale snapshot id), `src/snapshot/docker.rs` (malformed id, missing image), `src/snapshot/postgres.rs` (malformed snapshot id, missing dump), `src/snapshot/mysql.rs` (malformed snapshot id, missing dump), `src/snapshot/sqlite.rs` (malformed snapshot id, missing db artifact), `src/snapshot/supabase.rs` (malformed snapshot id, missing manifest dump), `tests/full_pipeline.rs` (known-provider malformed id, unknown-provider path); `rtk cargo test --lib`, `rtk cargo test --test full_pipeline rollback_with_`, `rtk cargo test --test snapshot_integration` | 2026-04-15 | 30 | true | Ilias | Pack 02: покрытие G7 расширено на matrix провайдеров; требуется добавить e2e подтверждение провайдер-специфичных rollback edge-cases в snapshot_integration/full_pipeline (если требуется). |
