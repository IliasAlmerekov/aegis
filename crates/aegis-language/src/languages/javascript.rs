...
/// bounded-resolution work and is deferred to a later slice, matching the
/// Python adapter's Slice 1 behavior. For a `template_string` with no
/// interpolation, the content is the concatenated `string_fragment` /
/// `escape_sequence` text (the text between the backticks).
